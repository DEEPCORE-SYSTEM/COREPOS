    ...
    <form action="{{ action('StockTransferController@update', [$sell_transfer->id]) }}" method="POST" id="stock_transfer_form">
                        ...
                        <div class="form-group">
                            ...
                            <select name="status" id="status" class="form-control select2" required>
                                ...
                                @foreach($statuses as $key => $status)
                                    <option value="{{ $key }}" {{ $sell_transfer->status == $key ? 'selected' : '' }}>{{ $status }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                        ...
                        <div class="form-group">
                            ...
                            <select name="location_id" id="location_id" class="form-control select2" disabled>
                                ...
                                @foreach($business_locations as $id => $location)
                                    <option value="{{ $id }}" {{ $sell_transfer->location_id == $id ? 'selected' : '' }}>{{ $location }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    <div class="col-sm-6">
                        <div class="form-group">
                            ...
                            <select name="transfer_location_id" id="transfer_location_id" class="form-control select2" disabled>
                                <option value="">{{ __('messages.please_select') }}</option>
                                @foreach($business_locations as $id => $location)
                                    <option value="{{ $id }}" {{ $purchase_transfer->location_id == $id ? 'selected' : '' }}>{{ $location }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="col-sm-10 col-sm-offset-1">
                        <div class="table-responsive">
                            <table class="table table-bordered table-striped table-condensed" id="stock_adjustment_product_table">
                                <thead>
                                    <tr>
                                        <th class="col-sm-4 text-center">{{ __('sale.product') }}</th>
                                        <th class="col-sm-2 text-center">{{ __('sale.qty') }}</th>
                                        <th class="col-sm-2 text-center">{{ __('sale.unit_price') }}</th>
                                        <th class="col-sm-2 text-center">{{ __('sale.subtotal') }}</th>
                                ...
                                <tfoot>
                                    <tr class="text-center">
                                        <td colspan="3"></td>
                                        <td>
                                            <div class="pull-right"><b>{{ __('stock_adjustment.total_amount') }}:</b> <span id="total_adjustment">{{ num_format($subtotal) }}</span></div>
                                        </td>
                                    </tr>
                                </tfoot>
                            </table>
        ...
        <div class="box box-solid">
            <div class="box-body">
                ...
                <div class="row">
                    <div class="col-sm-12">
                        <button type="submit" id="save_stock_transfer" class="btn btn-primary pull-right">{{ __('messages.save') }}</button>
                    </div>
                </div>
            </div>
        </div>
