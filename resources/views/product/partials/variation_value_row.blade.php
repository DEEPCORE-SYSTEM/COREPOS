...
<tr>
    <td>
        <input type="text" name="{{ $name }}[{{ $variation_index }}][variations][{{ $value_index }}][sub_sku]"
    ...
    <td>
        <input type="text" name="{{ $name }}[{{ $variation_index }}][variations][{{ $value_index }}][value]"
            value="{{ $variation_name }}" class="form-control input-sm variation_value_name" required {{ $readonly }} />
    </td>
    <td class="{{ $class }}">
        <div class="width-50 f-left">
            <input type="text"
                name="{{ $name }}[{{ $variation_index }}][variations][{{ $value_index }}][default_purchase_price]"
                value="{{ $default }}" class="form-control input-sm variable_dpp input_number"
        ...
        <div class="width-50 f-left">
            <div class="input-group">
                <input type="text"
                    ...
                    value="{{ $default }}" class="form-control input-sm variable_dpp_inc_tax input_number"
                ...
                <span class="input-group-btn">
                    <button type="button" class="btn btn-default bg-white btn-flat apply-all btn-sm p-5-5"
                        data-toggle="tooltip" title="{{ __('lang_v1.apply_all') }}"
    ...
    <td class="{{ $class }}">
        <div class="input-group">
            <input type="text"
                ...
                value="{{ $profit_percent }}" class="form-control input-sm variable_profit_percent input_number"
                required />
            @if($value_index == 0)
            <span class="input-group-btn">
                <button type="button" class="btn btn-default bg-white btn-flat apply-all btn-sm p-5-5"
    ...
    <td class="{{ $class }}">
        <input type="text"
            ...
            value="{{ $default }}" class="form-control input-sm variable_dsp input_number"
            placeholder="{{ __('product.exc_of_tax') }}" required />

        <input type="text"
            name="{{ $name }}[{{ $variation_index }}][variations][{{ $value_index }}][sell_price_inc_tax]"
            value="{{ $default }}" class="form-control input-sm variable_dsp_inc_tax input_number"
            placeholder="{{ __('product.inc_of_tax') }}" required />
    </td>
    <td>
        <input type="file" name="variation_images_{{ $variation_index }}_{{ $value_index }}[]" class="variation_images"
