<div class="col-sm-12"><br>
    ...
    <div class="col-sm-12">
        <div class="table-responsive">
            <table
                class="table table-condensed table-bordered table-striped table-responsive add-product-price-table combo_product_table">
                <thead>
                    <tr>
                ...
                <tbody>
                    ...
                    ['variations' => [$combo_variation['variation']], 'product' =>
                    $combo_variation['variation']->product, 'quantity' => $combo_variation['quantity'],
                ...
                <tfoot class="combo_product_table_footer">
                    <tr>
                        ...
                        <td class="text-center">
                            ...
                            <input type="hidden" name="item_level_purchase_price_total"
                                id="item_level_purchase_price_total" value="0">
        ...
        <div class="col-sm-12 col-sm-offset-4">
            <div class="col-sm-4">
                <label for="margin">{{ __('product.profit_percent') }}:</label>
                <input type="text" name="profit_percent" value="{{ number_format($profit_percent, 2) }}"
                    class="form-control input-sm input_number mousetrap" id="margin">
            </div>
            <div class="col-sm-4">
                <label for="selling_price">{{ __('product.default_selling_price') }}:</label>
                <input type="text" name="selling_price" value="{{ number_format(0, 2) }}"
                    class="form-control input-sm input_number mousetrap">

                <input type="hidden" name="selling_price_inc_tax" value="{{ number_format(0, 2) }}"
                    class="input_number mousetrap" id="selling_price_inc_tax">
            </div>
        </div>
                ...
                success: function(result) {
                    $(result).find('input.quantity').each(function() {
                        ...
                        $(".combo_product_table tbody").append(
                            update_combo_product_row_values(row));
                        update_net_total_amount();
                    });
                }
            });
    ...
    function update_net_total_amount() {
        ...
        $('.combo_product_table').find('tr').each(function() {
            if ($(this).find('input.item_level_purchase_price').length) {
                item_level_purchase_price_total += parseFloat($(this).find(
                    'input.item_level_purchase_price').val());
            }
        });
        ...
        //Set selling price.
        $(".combo_product_table").find('span.item_level_purchase_price_total').text(
            item_level_purchase_price_total);
        $(".combo_product_table").find('input#item_level_purchase_price_total').val(
            item_level_purchase_price_total);
