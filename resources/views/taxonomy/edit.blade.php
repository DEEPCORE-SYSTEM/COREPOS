    ...
    <form action="{{ action('TaxonomyController@update', [$category->id]) }}" method="POST" id="category_edit_form">
      ...
      <div class="modal-body">
        ...
        <div class="form-group">
          ...
          <input type="text" name="name" id="name" class="form-control" required placeholder="{{ $name_label }}" value="{{ $category->name }}">
        </div>

        @if($cat_code_enabled)
          <div class="form-group">
            <label for="short_code">{{ $cat_code_label }}:</label>
            <input type="text" name="short_code" id="short_code" class="form-control" placeholder="{{ $cat_code_label }}" value="{{ $category->short_code }}">
        ...
        <div class="form-group">
          ...
          <textarea name="description" id="description" class="form-control" placeholder="{{ __('lang_v1.description') }}" rows="3">{{ $category->description }}</textarea>
        </div>
