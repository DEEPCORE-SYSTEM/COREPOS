...
<section class="content">
    ...
    <form action="{{ action('DashboardConfiguratorController@update', ['id' => $dashboard->id]) }}" method="POST">
        @csrf
        @method('PUT')
...
$(document).ready(function() {
    ...
    $('div.row_option').click(function() {
        ...
        row_html = '<div class="row border-1px-173" data-ratio="' + ratio +
        ...
        ratio.split('-').forEach(function(ratio) {
            ...
            row_html = row_html + '<div class="droppable cell col-md-' + ratio +
                '"><div class="add_a_widget">' + '{{__("lang_v1.add_widget_here")}}' +
                '</div></div>';
        });
        row_html = row_html +
            '</div><div style="width:3% !important; display:inline; float:right; text-align:center"><i class="fas fa-grip-horizontal handle cursor-pointer" title="' +
            '{{__("lang_v1.move_row")}}' +
            '"></i> <i class="fas fa-times remove_row text-danger cursor-pointer" title="' +
            '{{__("lang_v1.delete")}}' + '"></i> </div></div>';
