<div class="modal-dialog" role="document">
	<div class="modal-content">
		...
		<div class="modal-body">
			...
			<div class="row">
				<div class="col-sm-9">
					<form action="{{ action('Restaurant\BookingController@update', [$booking->id]) }}" method="POST" id="edit_booking_form">
						@method('PUT')
						@csrf
						<div class="input-group">
							<select name="booking_status" class="form-control" required>
								<option value="" disabled>{{ __('restaurant.change_booking_status') }}</option>
								@foreach($booking_statuses as $key => $status)
								<option value="{{ $key }}" {{ $booking->booking_status == $key ? 'selected' : '' }}>{{ $status }}</option>
								@endforeach
							</select>
