        ...
        <div class="row">
            <div class="col-md-6">
                <div class="form-group">
                    <label for="group_by">@lang('lang_v1.group_sale_line_by'):*</label>
                ...
                <div class="scroll-top-bottom" style="max-height: 400px;">
                    <table class="table table-condensed table-striped">
                        ...
                        <tr>
                            ...
                            @foreach($row as $k => $v)
                            <td>
                                <select name="import_fields[{{ $k }}]" class="form-control import_fields select2"
                                    style="width: 100%;">
                                    <option value="">@lang('lang_v1.skip')</option>
                                    @foreach($import_fields as $key => $value)
                                    <option value="{{ $key }}" {{ $match_array[$k] == $key ? 'selected' : '' }}>
                                        {{ $value }}
                                    </option>
                                    @endforeach
                                </select>
