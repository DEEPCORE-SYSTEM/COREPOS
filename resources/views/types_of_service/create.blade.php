<div class="modal-dialog" role="document">
  <div class="modal-content">
    ...
    <!-- Formulario para agregar un nuevo tipo de servicio -->
    <form action="{{ action('TypesOfServiceController@store') }}" method="post" id="types_of_service_form">
      ...
      <div class="modal-header">
        ...
        <h4 class="modal-title">{{ __('lang_v1.add_type_of_service') }}</h4>
      </div>

      <div class="modal-body">
        <div class="row">

          <!-- Campo para el nombre del tipo de servicio -->
                    ...
                    <td>
                      <select name="location_price_group[{{ $key }}]" class="form-control input-sm select2" style="width: 100%;">
                        ...
                        @foreach($price_groups as $group_id => $group_name)
                          <option value="{{ $group_id }}" {{ old("location_price_group.$key") == $group_id ? 'selected' : '' }}>
                            {{ $group_name }}
                          </option>
                        @endforeach
                      </select>
          ...
          <div class="form-group col-md-6">
            ...
            <select name="packing_charge_type" id="packing_charge_type" class="form-control">
              <option value="fixed" {{ old('packing_charge_type', 'fixed') == 'fixed' ? 'selected' : '' }}>
                {{ __('lang_v1.fixed') }}
              </option>
              <option value="percent" {{ old('packing_charge_type') == 'percent' ? 'selected' : '' }}>
          ...
          <div class="form-group col-md-12">
            <div class="checkbox">
              <label>
                <input type="checkbox" name="enable_custom_fields" value="1" {{ old('enable_custom_fields') ? 'checked' : '' }}>
