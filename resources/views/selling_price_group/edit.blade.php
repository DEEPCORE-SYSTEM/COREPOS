        ...
        <form action="{{ action('SellingPriceGroupController@update', [$spg->id]) }}" method="POST" id="selling_price_group_form">
            ...
            <div class="modal-body">
                <div class="form-group">
                    ...
                    <input type="text" name="name" id="name" class="form-control" value="{{ $spg->name }}" required placeholder="@lang('lang_v1.name')">
                </div>

                <div class="form-group">
                    <label for="description">@lang('lang_v1.description'):</label>
                    <textarea name="description" id="description" class="form-control" rows="3" placeholder="@lang('lang_v1.description')">{{ $spg->description }}</textarea>
                </div>
            </div>
