        ...
        <div class="col-md-6">
            <div class="m-6 mt-5" style="display: flex;">
                <p><strong>@lang('sale.location'): &nbsp;</strong>
                    ...
                    <!-- Contenedor con estilo inline para controlar el ancho y el margen -->
                <div style="width: 28%; margin-bottom: 5px;">
                    <!-- Campo de selección de ubicación en Blade puro -->
                    <select name="select_location_id" id="select_location_id" class="form-control input-sm" required
                        ...
                        <!-- Generamos las opciones de ubicación dinámicamente -->
                        @foreach($business_locations as $id => $location)
                        <option value="{{ $id }}" <!-- Si la ubicación por defecto está definida y coincide, la marcamos
                            como seleccionada -->
                            @if(isset($default_location) && $default_location->id == $id) selected @endif
                            <!-- Agregamos atributos adicionales si existen -->
                            @foreach($bl_attributes ?? [] as $attr => $value) {{ $attr }}="{{ $value }}" @endforeach
                            >
                            {{ $location }}
                        </option>
                        @endforeach
