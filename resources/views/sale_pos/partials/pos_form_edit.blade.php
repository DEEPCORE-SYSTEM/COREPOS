...
@if(!empty($pos_settings['show_invoice_layout']))
    <div class="col-md-4">
        <div class="form-group">
            <select name="invoice_layout_id" class="form-control select2" id="invoice_layout_id">
                <option value="">@lang('lang_v1.select_invoice_layout')</option>
                @foreach($invoice_layouts as $key => $value)
                    <option value="{{ $key }}" {{ $transaction->location->invoice_layout_id == $key ? 'selected' : '' }}>
                        {{ $value }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="form-group">
            <select name="commission_agent" class="form-control select2">
                ...
                @foreach($commission_agent as $key => $value)
                    <option value="{{ $key }}" {{ $transaction->commission_agent == $key ? 'selected' : '' }}>
                        {{ $value }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="form-group">
            <div class="input-group">
                ...
                <input type="text" name="transaction_date" class="form-control" id="transaction_date" 
                    value="{{ @format_datetime($transaction->transaction_date) }}" readonly required>
            </div>
        </div>
    </div>
@endif
    ...
    <div class="col-md-4 col-sm-6">
        <div class="form-group">
            <div class="input-group">
                ...
                <input type="text" name="exchange_rate" class="form-control input-sm input_number"
                    id="exchange_rate" value="{{ @num_format($transaction->exchange_rate) }}"
    ...
    <div class="col-md-4 col-sm-6">
        <div class="form-group">
            <div class="input-group">
                ...
                <input type="text" name="price_group_text" class="form-control" value="{{ $transaction->price_group->name }}" readonly>
                <span class="input-group-addon">
    ...
    <div class="col-md-4 col-sm-6">
        <div class="form-group">
            <div class="input-group">
                ...
                <input type="text" name="types_of_service_text" class="form-control" value="{{ $transaction->types_of_service->name }}" readonly>
            ...
            <small>
                <p class="help-block {{ empty($transaction->selling_price_group_id) ? 'hide' : '' }}" id="price_group_text">
                    @lang('lang_v1.price_group'):
                    <span>{{ !empty($transaction->selling_price_group_id) ? $transaction->price_group->name : '' }}</span>
                </p>
            </small>
        </div>
    </div>
    ...
    <div class="col-sm-3">
        <div class="form-group">
            <select name="invoice_scheme_id" class="form-control">
                <option value="">@lang('lang_v1.select_invoice_scheme')</option>
                @foreach($invoice_schemes as $key => $value)
                    <option value="{{ $key }}" {{ $default_invoice_schemes->id == $key ? 'selected' : '' }}>
                        {{ $value }}
                    </option>
                @endforeach
            </select>
...
@if(in_array('subscription', $enabled_modules))
    <div class="col-md-4 col-sm-6">
        <label>
            <input type="checkbox" name="is_recurring" id="is_recurring" class="input-icheck" value="1" {{ $transaction->is_recurring ? 'checked' : '' }}>
