...
<tr @if(!empty($purchase_order_line)) data-purchase_order_id="{{$purchase_order_line->transaction_id}}" @endif>
    ...
    <td>
        ...
        <input type="hidden" name="purchases[{{ $row_count }}][purchase_order_line_id]"
            value="{{ $purchase_order_line->id }}">

        @endif

        <input type="hidden" name="purchases[{{ $row_count }}][product_id]" value="{{ $product->id }}">

        <input type="hidden" name="purchases[{{ $row_count }}][variation_id]" value="{{ $variation->id }}"
        ...
        $max_quantity = !empty($purchase_order_line) ? $purchase_order_line->quantity -
        $purchase_order_line->po_quantity_purchased : 0;
        @endphp
        ...
        <input type="text" name="purchases[{{$row_count}}][quantity]" value="{{@format_quantity($quantity_value)}}"
            ...
            data-rule-abs_digit={{$check_decimal}} data-msg-abs_digit="{{__('lang_v1.decimal_value_not_allowed')}}"
    ...
    <td>
        ...
        $purchase_order_line->pp_without_discount/$purchase_order->exchange_rate : $variation->default_purchase_price;
        ...
        $purchase_order_line->purchase_price/$purchase_order->exchange_rate : $variation->default_purchase_price;
        ...
        <input type="text" name="purchases[{{ $row_count }}][pp_without_discount]"
            value="{{ number_format($pp_without_discount, $currency_precision, $currency_details->decimal_separator, $currency_details->thousand_separator) }}"
    ...
    <td>
        <input type="text" name="purchases[{{ $row_count }}][discount_percent]"
            value="{{ number_format($discount_percent, $currency_precision, $currency_details->decimal_separator, $currency_details->thousand_separator) }}"
    ...
    <td>
        <input type="text" name="purchases[{{ $row_count }}][purchase_price]"
            value="{{ number_format($purchase_price, $currency_precision, $currency_details->decimal_separator, $currency_details->thousand_separator) }}"
    ...
    <td class="{{$hide_tax}}">
        <div class="input-group">
            <select name="purchases[{{ $row_count }}][purchase_line_tax_id]"
                ...
                <option value="{{ $tax->id }}" data-tax_amount="{{ $tax->amount }}" @if( $tax_id==$tax->id && $hide_tax
                    != 'hide') selected @endif >{{ $tax->name }}</option>
                @endforeach
            </select>
            <input type="hidden" name="purchases[{{ $row_count }}][item_tax]" value="0"
    ...
    <td class="{{$hide_tax}}">
        @php
        $dpp_inc_tax = number_format($variation->dpp_inc_tax, $currency_precision, $currency_details->decimal_separator,
        ...
        number_format($purchase_order_line->purchase_price_inc_tax/$purchase_order->exchange_rate, $currency_precision,
        $currency_details->decimal_separator, $currency_details->thousand_separator) : $dpp_inc_tax;

        @endphp
        <input type="text" name="purchases[{{ $row_count }}][purchase_price_inc_tax]" value="{{ $dpp_inc_tax }}"
    ...
    <td>
        <input type="text" name="purchases[{{ $row_count }}][lot_number]" class="form-control input-sm">
