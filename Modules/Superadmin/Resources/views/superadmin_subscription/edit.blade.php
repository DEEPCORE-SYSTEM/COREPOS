        ...
        <form action="{{ action('\Modules\Superadmin\Http\Controllers\SuperadminSubscriptionsController@update', $subscription->id) }}" method="POST" id="status_change_form">
            ...
            <div class="modal-body">
                <div class="form-group">
                    <label for="status">@lang("superadmin::lang.status")</label>
                    <select name="status" id="status" class="form-control">
                        @foreach($status as $key => $value)
                            <option value="{{ $key }}" {{ $subscription->status == $key ? 'selected' : '' }}>{{ $value }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="payment_transaction_id" id="payment_transaction_id" class="form-control" value="{{ $subscription->payment_transaction_id }}">
                </div>
            </div>
